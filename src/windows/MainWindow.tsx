...
import { AppWindow, Layers, Sparkles, Settings as SettingsIcon, ShieldCheck, ChevronRight } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { GeneralSettings } from "../components/Settings/GeneralSettings";

type Tab = "history" | "templates" | "ai" | "settings";
...
export default function MainWindow() {
  ...
  return (
    <div className="flex h-screen bg-zinc-50 dark:bg-[#1a1a1a] text-zinc-900 dark:text-zinc-200 overflow-hidden font-sans border border-zinc-200 dark:border-zinc-800 rounded-[12px] shadow-2xl">
      {/* Sidebar - macOS Style */}
      <aside className="w-[240px] flex flex-col bg-zinc-100/70 dark:bg-zinc-900/40 backdrop-blur-[40px] border-r border-zinc-200/30 dark:border-zinc-800/20 select-none">
        ...
        <div className="p-6 pt-2 pb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 opacity-60">
              ...
              <span className="text-[9px] font-bold uppercase tracking-widest">Version 1.0</span>
            </div>
          </div>
        </div>

        <nav className="flex-1 px-3 space-y-0.5">
          <p className="px-3 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-3 opacity-80">Library</p>
          <NavItem
...
function NavItem({ label, active, onClick, icon }: { label: string; active?: boolean; onClick: () => void; icon: React.ReactNode }) {
  return (
    ...
    >
      ...
      <span className={`${active ? "text-blue-600 dark:text-blue-400" : "text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300"} transition-colors`}>
        {icon}
      </span>
...
function EmptyState({ icon, title, tab }: { icon: React.ReactNode, title: string, tab: string }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
      ...
      <h2 className="text-xl font-bold tracking-tight font-display mb-1">{title}</h2>
      ...
      <div className="mt-8 flex items-center gap-2 group cursor-default">
        <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">Planned for</span>
